...
class FCN32s(nn.Module):
    ...
    def __init__(self, nclass, backbone='vgg16', aux=False, pretrained_base=True,
                 norm_layer=nn.BatchNorm3d, **kwargs):
...
class FCN16s(nn.Module):
    def __init__(self, nclass, backbone='vgg16', aux=False, pretrained_base=False, norm_layer=nn.BatchNorm3d, **kwargs):
        ...
        self.head = _FCNHead(512, nclass, norm_layer)
        self.score_pool4 = nn.Conv3d(512, nclass, 1)
        if aux:
            self.auxlayer = _FCNHead(512, nclass, norm_layer)
    ...
    def forward(self, x):
        ...
        upscore2 = F.interpolate(score_fr, score_pool4.size()[2:], mode='trilinear', align_corners=True)
        fuse_pool4 = upscore2 + score_pool4

        out = F.interpolate(fuse_pool4, x.size()[2:], mode='trilinear', align_corners=True)
...
class _FCNHead(nn.Module):
    def __init__(self, in_channels, channels, norm_layer=nn.BatchNorm3d, **kwargs):
